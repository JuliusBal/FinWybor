...
@section('title', $pageTitle ?? 'Kredyty gotówkowe – porównaj oferty')
@section('meta_description', $pageDescription ?? 'Sprawdź RRSO, ratę, całkowity koszt i czas wypłaty.')
@isset($canonical)
    @push('head')
...
@section('content')
        ...
        <div class="max-w-7xl mx-auto px-4 md:px-6 py-8">
            <nav class="text-xs text-white/70 mb-2" aria-label="breadcrumbs">
                <ol class="flex items-center gap-2">
                    ...
                    <li aria-hidden>›</li>
                    <li class="text-white/90">Kredyty gotówkowe</li>
                </ol>
            </nav>
            <h1 class="text-2xl md:text-3xl font-bold tracking-tight">Kredyty gotówkowe</h1>
            <p class="mt-2 text-sm md:text-base text-white/85">
                Porównaj oferty, sprawdź RRSO, ratę i czas wypłaty. Wybierz najlepszy kredyt gotówkowy dla siebie.
            </p>
        </div>
    </section>

    <div class="max-w-7xl mx-auto px-4 md:px-6 -mt-6">
        <form id="offers-filter" method="get" action="{{ $baseAction }}"
            ...
            <div class="grid grid-cols-2 md:grid-cols-12 gap-3">
                ...
                <div class="md:col-span-3">
                    ...
                    <input type="number" name="amount" value="{{ $amount }}"
                           ...
                           min="500" step="100" placeholder="np. 3000">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" name="term" value="{{ $term }}"
                           class="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400"
                           min="1" max="120" placeholder="np. 12">
                </div>
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="sort"
                            class="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400">
                        <option value="fastest"  @selected($sort==='fastest')>Najszybsza wypłata</option>
                        <option value="cheapest" @selected($sort==='cheapest')>Najtaniej (nadpłata)</option>
                        <option value="brand"    @selected($sort==='brand')>Alfabetycznie</option>
                ...
                <div class="col-span-2 md:col-span-12 order-last md:order-none">
                    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                        <div class="flex flex-wrap items-center gap-2">
                            <a data-ajax href="{{ $baseAction.'?'.http_build_query(array_merge(request()->except('sort'), ['sort'=>'fastest'])) }}"
                               class="px-3 py-1.5 rounded-lg border border-brand-800/20 text-brand-800 bg-brand-800/5 hover:bg-brand-800/10 text-xs">
                                Najszybsza wypłata
                            </a>
                            <a data-ajax href="{{ $baseAction.'?'.http_build_query(array_merge(request()->except('sort'), ['sort'=>'cheapest'])) }}"
                               class="px-3 py-1.5 rounded-lg border border-brand-800/20 text-brand-800 bg-brand-800/5 hover:bg-brand-800/10 text-xs">
                                Najtaniej
                            </a>
                        </div>
        ...
        <div id="offers-surface" class="mt-6 relative">
            ...
            <div id="offers-results" class="min-h-[360px]">
                @include('offers._list', ['offers' => $offers])
            </div>
        </div>
    ...
    @includeIf('static._mini_faq_loans')
    ...
    @push('scripts')
        @verbatim
            ...
            {"@type":"ListItem","position":2,"name":"Kredyty gotówkowe","item":"{{ url('/kredyty-gotowkowe') }}"}
          ]
        }
            </script>
        @endverbatim
